...
static func lerp_list(a: Array, t: float) -> Variant:
	if a.size() == 0: return 0.0
	if a.size() == 1: return a[0]
	t = wrapf(t, 0.0, 1.0)
	var scaled: float = t * float(a.size() - 1.0)
	
	var from = a[wrapi(int(floor(scaled)), 0, len(a))]
	
	var to = a[wrapi(int(floor(scaled + 1.0)), 0, a.size())]
...
func _process_custom_fx(char_fx: CharFXTransform) -> bool:
	...
	var c1: Color = char_fx.env.get("c1", char_fx.color)
	...
	if colors.size() != 0:
		var t: float = sin(char_fx.elapsed_time * freq + get_rand_unclamped(char_fx)) * .5 + .5
